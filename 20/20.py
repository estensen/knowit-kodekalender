nums = [7096, 3, 3924, 2404, 4502, 4800, 74, 91, 9, 7, 9, 6790, 5, 59, 9, 48, 6345,
...
def largest_possible_number(nums):
    nums_str = [str(s) for s in nums]
    max_num_str_len = max(len(s) for s in nums_str) * 2  # double the length of the longest snippet
    # Extend shorter numbers so they can be compared with longer ones
    return ''.join(sorted(nums_str, reverse=True, key=lambda s: s * (max_num_str_len // len(s) + 1)))
